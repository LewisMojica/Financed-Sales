...
- Follow standard conventions for Frappe/ERPNext development
- Use descriptive messages that explain business functionality changes
- For workflow development, mention which agents or testing levels are affected

## Custom Formatting Commands
```bash
# Python formatting for Frappe apps
test -f pyproject.toml && ruff format .
test -f pyproject.toml && ruff check --fix .
```

## Repository-Specific Rules
- This is a Frappe app repository for financed sales functionality
- Changes may include DocType modifications, API functions, or workflow development
- Test files should be included in commit scope when testing is modified